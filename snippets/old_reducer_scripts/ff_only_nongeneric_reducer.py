...
# def single_layer_load(h5_loc, i, dr, skips=4):
...
def cc3d_feature_finder(data_slice,
    ...
    del feature_map, inv, counts, idxs
...
data = dict([(k, np.zeros(subpanel_shape + (2, 0, 0))) for k in med.keys()])
...
for future in as_completed(futures):
    ...
    if consec_loaded-explored_layers < 30 and consec_loaded < n_frames:
        continue
    slices = dict([(k,data[k][explored_layers:consec_loaded]) for k in keys])
    if consec_loaded == n_frames:
        slices = dict([(k,data[k][explored_layers:]) for k in keys])
    ...
    for f2 in as_completed(f2s):
        ...
        k = out[-1]
        spots[k] = np.vstack([spots[k], out[0]+[0, explored_layers,0,0]])
        coords[k] = np.vstack([coords[k], out[1]+[explored_layers,0,0]])
        vals[k] = np.vstack([vals[k], out[2]])
        fids[k] = np.vstack([fids[k], out[3]])
        explored_layers = consec_loaded - out[4]
